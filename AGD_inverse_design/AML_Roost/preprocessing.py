...
class preprocessing:
    def __init__(self, filepath, dataset):
        ...
        if dataset is 'bandgap':
            ...
            if os.path.exists('./bd_AML_whole_init_1000.csv') is False:
                # small examples for debugging
                self.df = self.df.sample(n=1000, replace=True, random_state=42)
                added_columns_name = []
                for i in range(128):
                ...
                for formula in self.df['composition']:
                    print(formula)
                ...
                self.df.to_csv(r'bd_AML_whole_init_1000.csv', index=False, header=True)

            else:
                self.df = pd.read_csv('./bd_AML_whole_init_1000.csv')
...
# p = preprocessing()
# #
# # x_train, x_test, y_train, y_test, med, top10 = p.read_superconduct_data()
# drange = p.get_range_dic()
# # p.get_superconduct_realworld_value([])
# print()
#
